...
for i in out:
    transform_data = data[i]**(1/2)
    transform_data2 = test[i]**(1/2)
...
test = pd.DataFrame(test, columns = data.columns[0:10])
'''
'''
...
for i in out:
    transform_data = X_df[i]**(1/3)
#    transform_data2 = test[i]**(1/2)
#    transform_data, lam = boxcox(X_df[i])
#    transform_data2, lam = boxcox(test[i])
    for j in range(0, X_df.shape[0]):
        X_df.loc[j, i] = transform_data[j]
#    for j in range(0, test.shape[0]):
#        test.loc[j, i] = transform_data2[j]
'''
